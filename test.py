...
save_path = ("D:\cv2\renlian")
face_cascade = cv2.CascadeClassifier("D:\Software\Python37\Lib\site-packages\cv2\data\haarcascade_frontalface_default.xml")#'./cascades/haarcascade_frontalface_default.xml'
eye_cascade = cv2.CascadeClassifier("D:\Software\Python37\Lib\site-packages\cv2\data\haarcascade_eye.xml")#'./cascades/haarcascade_eye.xml'
...
while True:
    ...
    if pre_frame is None:
        ...
        thresh = cv2.dilate(thresh, None, iterations=2)#image,
        ...
        for c in contours:
            if cv2.contourArea(c) < 1000: # 设置敏感度
                ...
                print("咦,有什么东西在动0.0")
                playsound("D:\cv2\jingbao.mp3")#找到playsound源码把.encode('utf-16')去掉就可以运行了Windows是（gbk）.encode("gbk")
                break
